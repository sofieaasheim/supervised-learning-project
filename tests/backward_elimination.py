...
df.drop(["Country", "Year", "Status", "PercentageExpenditure"], axis=1, inplace=True)
...
# UnderFiveDeathsand Thinness 5_9 also seemed to have a slight linear relationship with the
# response, but these parameters are highly correlated with InfantDeaths
...
test3_parameters.remove("HepatitisB")
...
test4_parameters.remove("InfantDeaths")
...
# AdultMortality, Alcohol, GDP, BMI, TotalExpenditure, HIVAIDS, Diphtheria, Income, Schooling

# We can also try to remove the next parameter with the highest p-value to see if R^2 improves
# (it is now 0.820 and adj. R^2 is 0.819):

# The next highest p-value parameter is TotalExpenditure
test5_parameters = test4_parameters
test5_parameters.remove("TotalExpenditure")
...
# (it is now 0.820 and adjusted is 0.818.)
